...
**Maszyna:** maszyna pomiarowa (dostęp jak w zleceniach 02–06).
...
```
C:\Users\operator\spektra1-env\Scripts\python.exe -c "import pandas as pd; s=pd.read_parquet('measurements-glowny/spectra.parquet'); [s[s.language==l].to_parquet(f'measurements-glowny/spectra-{l}.parquet', index=False) or print(l, len(s[s.language==l])) for l in sorted(s.language.unique())]"
...
Zakres zmian: wyłącznie wewnątrz `C:\Users\operator\spektra1\`. Rollback: bez
zmian; nowe pliki `spectra-{en,pl}.parquet` można skasować po potwierdzeniu
odbioru.

## Po zaliczeniu

Czat prowadzący uruchamia lokalnie `python -m gates.gate2`. Kod jest gotowy
i przetestowany (129 testów) **od 2026-08-01, przed istnieniem tych progów** —
