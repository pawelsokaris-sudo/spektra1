...
- Semantyka `hidden_states`: **zweryfikowana hookami 2026-07-30** (`docs/layer_semantics.md`, T2). Ustalenia wchodzące do pieczęci:
  - `hidden_states` ma **35 elementów** przy **34 blokach**. `hidden_states[0]` to embedding — **wyłączony z pasm**.
  - `hidden_states[ℓ+1]` = wyjście bloku ℓ, surowe (przed final norm), **potwierdzone co do zera dla bloków 0–32**.
  - **Wyjątek: `hidden_states[34]` jest już po final norm** — rozbieżność wobec surowego wyjścia bloku 33 znika dokładnie do zera po zastosowaniu normalizacji. To znana własność implementacji, nie błąd.
  - **Konsekwencja: `hidden_states[34]` jest wyłączony z pomiaru.** Leży poza pasmem [0.4L, 0.8L], więc decyzja nie dotyka analizy głównej. Mieszanie go z warstwami surowymi byłoby błędem niezależnie od pasma: warstwy surowe mają wartości rzędu 1.5–2.9·10⁵, znormalizowana ostatnia rzędu 10² — statystyki byłyby zdominowane różnicą skali, nie treścią.
  - Pasmo [0.4L, 0.8L] obejmuje **bloki 13–26** (14 bloków), czyli `hidden_states[14..27]`. Wszystkie surowe, wszystkie potwierdzone.
  - Mapa typów bloków: uwaga **globalna w blokach 5, 11, 17, 23, 29**, pozostałe lokalne (wzorzec 5:1). Skład pasma: **2 globalne, 12 lokalnych** (14,3% globalnych wobec 14,7% w całym modelu — pasmo jest reprezentatywne). Typ bloku wchodzi jako kowariancja do analiz wtórnych.
  - **Kontrola szablonu czatu zaliczona:** generator korpusu produkuje znak w znak ten sam tekst co `tokenizer.apply_chat_template`. Rozbieżność oznaczałaby, że mierzymy inny tekst niż zapisany w scenariuszach.
- Precyzja: forward w **bf16**; kontrola: reprezentatywne 10% tekstów także w fp32; zamrożona tolerancja |ΔI_total| < 0.005 między dtype — przekroczenie = stop i diagnoza. Kontrola fp32 wykonywana **na tym samym urządzeniu** (GPU) mimo że przekracza pamięć karty i przelewa się do RAM: kontrola ma izolować wyłącznie dtype, a przeniesienie jej na CPU zmieniałoby jednocześnie dtype i urządzenie.
- **Bramka pamięci (obowiązkowa).** Sterownik NVIDIA na maszynie pomiarowej ma włączony *system memory fallback* — przy braku pamięci karty cicho dokłada pamięć systemową zamiast zgłosić błąd (zweryfikowane wprost: alokacja 20 GB na karcie 15,92 GB przechodzi). Oznacza to, że **brak błędu CUDA nie jest dowodem, że bieg zmieścił się w zapieczętowanych warunkach**. Po każdym biegu kod sprawdza `torch.cuda.max_memory_allocated()` wobec zamrożonego progu (14 GB dla pomiaru głównego, 24 GB dla kontroli fp32) i przerywa przy przekroczeniu. Progi w `config.yaml`, implementacja: `pipeline/memory_guard.py`. Zmierzony szczyt pomiaru głównego przy budżecie 1024 tokenów: 8,83 GB. Kanały o wariancji < ε = 1e-6 (po z-score przed nim: wariancja surowa) wykluczane wg jawnej reguły, liczba raportowana.
