# ANEKS 4 — nieosiągalny margines równoważności przy M = 24

**DECYZJA KIEROWNIKA BADANIA: opcja (A) — 2026-08-01.**
Margines |d_z| < 0.3 i α_TOST = 0.05 na stronę **pozostają niezmienione**.
Przyjmujemy do wiadomości, że przy M = 24 są nieosiągalne. Żaden zamrożony
parametr nie został tknięty po zebraniu danych.

**Ten aneks zastępuje jedno zdanie §0 protokołu.** Zdanie zapieczętowane:
*„Wyniki mają trzy możliwe werdykty (nie »falsyfikację«): (a) efekt wykryty;
(b) efekt praktycznie wykluczony (test równoważności); (c) wynik
niekonkluzywny."* Werdykt (b) **nie jest osiągalny w SPEKTRA-1** — zbiór
werdyktów to {(a) efekt wykryty, (c) niekonkluzywny}. Sam protokół pozostaje
NIEZMIENIONY co do bajtu: po pieczęci zmiany wyłącznie aneksem, a plik
protokołu jest dowodem, że treść naukowa nie była ruszana (por. ANEKS-3).

## Zobowiązania raportowe wynikające z decyzji

1. Jeśli H1 wyjdzie nieistotne, raport podaje **wprost**: *przy M = 24 nie
   odróżnilibyśmy braku efektu od zbyt małej czułości badania* — nie wolno
   napisać „efektu nie ma" ani „nie wykazano równoważności" jako wyniku.
2. **H4 traci możliwość potwierdzenia.** Raportowana opisowo: zmierzona
   wielkość efektu + 99% CI, bez orzeczenia o równoważności.
3. Raport podaje **minimalny osiągalny margines przy M = 24 (≈0.4)** oraz
   liczność potrzebną dla marginesu 0.3 (**32** granicznie, **64** z zapasem)
   — jako kalibrację dla SPEKTRA-2, nie jako usprawiedliwienie.
4. Interpretację werdyktu „niekonkluzywny" niesie zamrożona w ANEKS-2 analiza
   czułości (minimalny wykrywalny efekt przy M = 24 i mocy 0.80).

## Uzasadnienie wyboru (A) zamiast poszerzenia marginesu

Poszerzenie do |d_z| < 0.5 byłoby dopuszczalne — decyzja zapadałaby przed
odczytaniem czegokolwiek i byłaby jawnie zapisana. Odrzucone, bo recenzent ma
prawo przeczytać zmianę zamrożonego progu po zebraniu danych jako dobieranie
go pod wynik, a nietknięta prerejestracja jest jedynym kapitałem, którego
temu badaniu nie da się odebrać. Zawężenie pytania („czy efekt jest" zamiast
„czy efektu na pewno nie ma") jest realnym kosztem — drugie pytanie i tak było
poza zasięgiem tej próby od pierwszego dnia, tylko nikt tego nie sprawdził.

---

# Znalezisko (zapis pierwotny, 2026-08-01, przed decyzją)
...
**Decyzja kierownika badania:** opcja (A), 2026-08-01 — zapisana na górze tego aneksu.
